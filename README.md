...
There is no auto-update. When a new version is posted here, download the zip
again, replace the files you unzipped before, and click the reload icon on the
extension's card in `chrome://extensions`.

## If Chrome turns it off

Chrome sometimes disables extensions loaded this way after a restart, and will
say so on `chrome://extensions`. Switching it back on there is enough — the
files are still where you left them.
